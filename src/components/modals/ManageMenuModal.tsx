    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-background border border-border rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col animate-slide-up">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div>
            <h2 className="text-xl font-black text-foreground">Manage Menu</h2>
            <p className="text-xs text-muted-foreground mt-1">Add items, edit, or mark out of stock</p>
          </div>
          <button
            onClick={onClose}
              ...
              {filteredItems.map((item) => (
                <div
                  ...
                  className={`bg-card border rounded-xl p-4 flex items-center justify-between hover:shadow-md transition-shadow ${
                    !item.available ? 'opacity-60 border-amber-500' : 'border-border'
                  }`}
                >
